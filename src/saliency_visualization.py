...
class SaliencyMap:
    def compute_saliency_maps(self, X, y, model):
        ...
        ##############################################################################
        y_oh = torch.nn.functional.one_hot(y_var, num_classes=100)
    ...
    def show_saliency_maps(self, X, y, labels, model, folder="l1_unstructured", suffix="Full", verbose=True):
        ...
        X_tensor = torch.cat([preprocess(Image.fromarray(x)).unsqueeze(0) for x in X], dim=0).to(device)
        ...
        plt.savefig(f"./visualization/saliency_viz/{folder}/{suffix}.png")
        
        if verbose:
            plt.show()

    def run_program(self, model, prune_config, global_prune=False, verbose=False, retrain=False, folder="l1_unstructured", suffix="Full"):
        pruner = Pruner(model, prune_config, global_prune)
        self.pruner = pruner
        model = self.pruner.prune_model(retrain=retrain)
        ...
        self.show_saliency_maps(X, y, labels, model, verbose=verbose, folder=folder, suffix=suffix)
